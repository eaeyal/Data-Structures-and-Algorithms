...
public class MaxHeap<T extends Comparable<? super T>> {
    ...
    public void add(T data) {
        if (size + 1 >= backingArray.length) {
            T[] newBacker = (T[]) new Comparable[2 * backingArray.length];
            int i = 0;
            for (T element : backingArray) {
                newBacker[i] =  backingArray[i];
                i++;
            }
            backingArray = newBacker;
        }
        size++;
    ...
    public T remove() {
        if (size == 0) {
            throw new NoSuchElementException("You cannot remove from an empty heap!");
        }
        ...
        int i = 1;
        while (i * 2 <= size) {
            if (i * 2 + 1 <= size && backingArray[i].compareTo(backingArray[2 * i + 1]) < 0 && backingArray[2 * i].compareTo(backingArray[2 * i + 1]) < 0) {
